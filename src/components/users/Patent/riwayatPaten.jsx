...
import EditPatentPopup from "./editPatentPopup";
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await axios.get(
      `${API_URL}/submissions/patent/user/${userId}`,
      {
        headers: {
...
const filteredData = dataRiwayat.filter((item) => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 mt-10 bg-white rounded-lg shadow-lg shadow-gray-400">
      <h1 className="text-xl font-bold mb-4">Riwayat Pengusulan Paten</h1>
...
{showBuktiPembayaranColumn && (
  <td className="px-4 py-2 border-b">
    {item.status.toLowerCase() === "submit" ? (
      item.buktiPembayaran && item.buktiPembayaran.toLowerCase() !== "tidak ada bukti pembayaran" ? (
        <FileLinkWithPreview fileUrl={item.buktiPembayaran} label="Lihat Bukti" />
      ) : (
  ...
  <div className="flex flex-wrap gap-2 justify-start">
    {item.status.toLowerCase() === "submit" ? (
      ...
      <>
        ...
        <button
          onClick={() => handleEditFile(item.patentId)}
